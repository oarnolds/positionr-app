...
- **Correcties tijdens uitvoering (Task 4):** de EPUB-extractie in dit plan had drie bugs die bij de implementatie zijn gefixt: (1) metadata staat in de `dc:`-namespace (`dc:title`/`dc:creator`/`dc:language`) — de selector werd `dc\:title, metadata > title`; (2) `.text()` plakte blok-elementen (h1+p) aan elkaar — blok-elementen krijgen nu een newline; (3) de woorddrempel van 50 filterde echte korte hoofdstukken weg — verlaagd naar 5. Geverifieerd op het echte Cialdini-EPUB: titel/auteur/taal correct, 18 hoofdstukken.
