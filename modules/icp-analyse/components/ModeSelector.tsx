...
import { startMarkdownAnalyse } from "@/app/(app)/modules/icp-analyse/actions";

export type SnapshotOption = { id: string; label: string };

// Live scraping is uit het proces gehaald (besluit juli 2026): analyses
// draaien altijd op een markdown-snapshot uit de bibliotheek. De
// "Volledige analyse" blijft zichtbaar maar is nog niet beschikbaar.
export function ModeSelector({
  ...
  const startDisabled = pending || !snapshotId;
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        <ModeCard
          ...
          bullets={["Klaar in ~30 sec", "Rijkste bron: alle pagina's", "Kies zelf de bron"]}
        />
        <ModeCard
          active={false}
          icon={<ClipboardList className="h-5 w-5 text-blue-600" />}
          title="Volledige analyse"
          subtitle="Markdown-analyse gecombineerd met vragenlijst voor scherper profiel."
          bullets={["Meest nauwkeurig", "Markdown + vragenlijst", "~10-15 min"]}
          comingSoon
        />
      </div>
